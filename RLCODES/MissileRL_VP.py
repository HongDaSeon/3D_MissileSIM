...
def Transmit2Vpython(MissileInstance, MissileSeekerInstance, t, td_reward):
    ...
    if vpythonsim & (not done):
        R, LOOK, dLOS, Scalvel, _ = MissileSeekerInstance.seek()
...
g1 = vp.graph(title = "LOOKazim",width = 500, height = 200)
g2 = vp.graph(title = "LOOKelev",width = 500, height = 200)
...
#=======================================================================
# Graph Initilize
LOOK_azim_graph  = vp.gcurve(graph = g1, color = vp.color.black)
LOOK_elev_graph  = vp.gcurve(graph = g2, color = vp.color.black)

R_graph     = vp.gcurve(graph = g3, color = vp.color.magenta)
accy_graph  = vp.gcurve(graph = g4, color = vp.color.black)
accz_graph  = vp.gcurve(graph = g5, color = vp.color.black)
mu_graph    = vp.gcurve(graph = g4, color = vp.color.red)
rwd_graph   = vp.gcurve(graph = g6, color = vp.color.blue)
stt_graph   = vp.gcurve(graph = g7, color = vp.color.green)
...
missile_visual = vp.sphere(axis = MissileModel_1.dpos.VPvec, size = vp.vec(2.01,0.5,0.5), color = vp.color.red,
                           accaxis = MissileModel_1.acc.VPvec, make_trail = True, retain = 600)
...
for i_ep in range(10000):
    ...
    while t<max_step_count:
        ...
        Transmit2Vpython(MissileModel_1, MissileSeeker_1, t, Mt_reward)
    ...
    if(i_ep % 2 == 0 & vpythonsim):VpythonClearGraph()
